...
```bash
cargo test -p hello-wasm -- --nocapture
```

当你运行这个命令时，Cargo 会在 hello-wasm 包中运行所有标记为测试的代码（通常是以 `#[test]` 属性标记的函数），并且不会捕获这些测试的输出。这对于调试测试非常有用，因为你可以直接看到测试函数中的 `println!` 或其他打印语句的输出。
